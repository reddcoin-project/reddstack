...
class BlockstoredRPC(jsonrpc.JSONRPC, object):
    ...
    def jsonrpc_register_tx_unsigned( self, name, user_public_key, register_addr ):
        ...
        Return a JSON object with 'error' on error.
        """
        return blockstore_name_register( str(name), None, str(register_addr), tx_only=True, user_public_key=str(user_public_key), testset=self.testset )
    ...
    def jsonrpc_get_name_cost( self, name ):
        ...
        return {"satoshis": int(math.ceil(name_cost)), "est_fees": int(math.ceil(fees)), "opt_subsidy": int(math.ceil(donation)), "est_totalcost": int(math.ceil(total))}
