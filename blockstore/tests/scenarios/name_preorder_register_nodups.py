...
def check( state_engine ):
    ...
    preorder = state_engine.get_name_preorder( "foo.test", pyreddcoin.make_pay_to_address_script(wallets[4].addr), wallets[5].addr )
    if preorder is None:
        return False 

    preorder = state_engine.get_name_preorder( "foo.test", pyreddcoin.make_pay_to_address_script(wallets[0].addr), wallets[1].addr )
    if preorder is None:
        return False
    
    preorder = state_engine.get_name_preorder( "foo.test", pyreddcoin.make_pay_to_address_script(wallets[5].addr), wallets[2].addr )
    if preorder is None:
        return False
