...
def do_interleaving( name, namerecs, order ):
    ...
    for i in xrange(0, len(order)):
        ...
        if op == 'r':
            # renew
            print "\nrenew '%s' with %s\n" % (name, pyreddcoin.make_pay_to_address_script( namerecs[name][0].addr ))
            ...
            print "\ntransfer '%s' from %s to %s" % (name, pyreddcoin.make_pay_to_address_script( namerecs[name][0].addr ), pyreddcoin.make_pay_to_address_script( namerecs[name][1].addr ))
...
def check( state_engine ):
    ...
    for name in names.keys():
        ...
        # check owner 
        if name_rec['address'] != names[name]['address'] or name_rec['sender'] != pyreddcoin.make_pay_to_address_script( names[name]['address'] ):
